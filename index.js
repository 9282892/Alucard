	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oiin @${num.split('@')[0]}\nBem vindo ao grupo leia as regras para n�o levar ban *${mdata.subject}*`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				error: {
					stick: 'FUDEU N�O CONSEGUI CONVERTER A IMAGEM EM FIGURINHA ',
					Iv: '❌ Link inválido ❌'
				},
				only: {
					...
					admin: '[❗] Este comando so pode ser usado pelos ademir do grupos',
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nOlá como!\nVocê não está registrado no banco de dados, \n\nComando : ${prefix}registrar nome|idade\nExemplo : ${prefix}registrar Alucard|18`,
				}
			}
               ...
               break
				case 'rebaixar':
                  ...
                  case 'promover':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Esse bixin aque agora  � adm :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'remover':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Tchauzinho otario vagabundo :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Tchauzinho otario vagabundo : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('O comando ja esta ativo')
						welkom.push(from)
